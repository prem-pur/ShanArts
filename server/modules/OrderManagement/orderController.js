...
const orderController = {
    ...
    async assignOrder(req, res, next) {
        try {
            const { assignedOperatorId, assignedMachineId, estimatedCompletionTime } = req.body;
            const order = await ShopOrder.findById(req.params.id);
    ...
    async submitToCustomer(req, res, next) {
        try {
            ...
            await notificationService.createNotification(
                ...
                `The design for order #${order.orderNumber} is ready. Please review and approve.`,
                order._id,
                'Order'
            );

            res.json({ message: 'Design submitted for customer approval', order });
        } catch (error) {
            next(error);
        }
    },
