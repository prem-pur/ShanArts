...
const AddOrder = ({ onOrderCreated, onCancel }) => {
    ...
    useEffect(() => {
        ...
        if (distance !== formData.address.distance) {
            setFormData(prev => ({
                ...prev,
    ...
    const checkDistance = () => {
        setError(''); // Clear error before check
        // Phone validation
        ...
        if (formData.deliveryMethod !== 'pickup') {
            if (!formData.address.street.trim() || !formData.address.city.trim()) {
                setError('Please provide a complete delivery address (Street and City).');
                return;
            }
            if (formData.address.distance > 10) {
                setError('Delivery is not available for this distance. Please choose Pickup.');
                return;
            }
        }
                ...
                <form onSubmit={handleSubmit}>
                    {step === 1 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            ...
                            <div>
                                <label style={labelStyle}>Phone Number (07XXXXXXXX)</label>
                                <input
                                    type="text" name="customerPhone" placeholder="0XXXXXXXXX"
