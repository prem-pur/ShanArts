...
const OperatorWorkspace = () => {
    ...
    return (
        <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', fontFamily: "var(--font-sans, sans-serif)", backgroundColor: 'var(--bg-color)', minHeight: '100vh', color: 'var(--text-primary)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                <div>
                    <h2 style={{ fontSize: '32px', fontWeight: '900', color: 'var(--text-primary)', marginBottom: '8px' }}>Operator Workspace</h2>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '16px' }}>View and manage your assigned printing tasks.</p>
                </div>
                <div style={{
            ...
            <div style={{ background: 'var(--card-bg)', borderRadius: '20px', padding: '32px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-color)', marginBottom: '32px' }}>
                ...
                {tasks.filter(t => t.status !== 'completed').length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '24px' }}>
                        {tasks.filter(t => t.status !== 'completed').map(task => {
                            ...
                            return (
                                <div key={task._id} style={{ border: '1.5px solid var(--border-color)', borderRadius: '16px', padding: '24px', transition: 'transform 0.2s', background: 'var(--card-bg)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                                        <div>
                                            <div style={{ fontWeight: '900', fontSize: '18px', color: 'var(--text-primary)' }}>#{task.orderNumber || task.orderId}</div>
                                            <div style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: '600', textTransform: 'uppercase' }}>{jobType}</div>
                                        </div>
                                        <span style={{
                                            padding: '4px 10px',
                                            borderRadius: '6px',
                                            background: task.status === 'confirmed' ? '#fee2e2' : isPrinting ? '#fee2e2' : '#fff7ed',
                                            color: task.status === 'confirmed' ? '#ff3333' : isPrinting ? '#cc0000' : '#ea580c',
                                            fontSize: '11px',
                                            fontWeight: '800',
                                    ...
                                    <div style={{ background: 'var(--surface-muted)', padding: '12px', borderRadius: '8px', marginBottom: '16px', border: '1px solid var(--border-color)' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                                            <span style={{ fontSize: '16px' }}>{machineIcon}</span>
                                            <div style={{ fontSize: '11px', fontWeight: '800', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Assigned Machine</div>
                                        </div>
                                    ...
                                    <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '20px', lineHeight: '1.5' }}>{task.printSpecs?.description || task.description || 'No additional instructions.'}</p>
                                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '20px', display: 'flex', flexWrap: 'wrap', gap: '12px', fontWeight: '600' }}>
                                    ...
                                    {!isPrinting ? (
                                        <button
                                            ...
                                            style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', background: 'var(--accent-color)', color: '#fff', fontWeight: '800', cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 12px var(--accent-glow)' }}
                                        ...
                                        <button
                                            ...
                                            style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', background: 'var(--text-primary)', color: 'var(--bg-color)', fontWeight: '800', cursor: 'pointer', transition: 'all 0.2s', boxShadow: 'var(--shadow-md)' }}
                                            onMouseOver={(e) => e.target.style.transform = 'translateY(-2px)'}
                                            onMouseOut={(e) => e.target.style.transform = 'translateY(0)'}
                                        >
                                            ✅ MARK COMPLETED
                ...
                {tasks.filter(t => t.status === 'completed').length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '24px' }}>
                        {tasks.filter(t => t.status === 'completed').map(task => {
                            ...
                            return (
                                <div key={task._id} style={{ border: '1.5px solid var(--border-color)', borderRadius: '16px', padding: '24px', background: 'var(--surface-muted)', opacity: 0.8 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                                        <div>
                                            <div style={{ fontWeight: '900', fontSize: '18px', color: 'var(--text-primary)' }}>#{task.orderNumber || task.orderId}</div>
                                            <div style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: '600', textTransform: 'uppercase' }}>{jobType}</div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
                    <div style={{ position: 'relative', background: '#fff', width: '100%', maxWidth: '600px', borderRadius: '24px', padding: '40px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
                        <div style={{ marginBottom: '32px' }}>
                            <h3 style={{ fontSize: '24px', fontWeight: '900', color: 'var(--text-primary)', marginBottom: '8px' }}>Log Material Usage</h3>
                            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', fontWeight: '600' }}>Please record the materials used for order #{selectedTask.orderNumber} before completion.</p>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '350px', overflowY: 'auto', marginBottom: '24px', paddingRight: '8px' }}>
                            {materialsUsed.map((row, index) => {
                                ...
                                return (
                                    <div key={index} style={{ display: 'flex', gap: '12px', alignItems: 'center', background: 'var(--surface-muted)', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <button
                                        ...
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                            <div style={{ fontSize: '15px', fontWeight: '700', color: selectedMaterial ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                        ...
                        <button
                            ...
                            style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1.5px dashed #e5e7eb', background: 'none', color: 'var(--text-secondary)', fontWeight: '700', cursor: 'pointer', marginBottom: '32px' }}
                        >
                            + ADD ANOTHER MATERIAL
                        </button>

                        <div style={{ display: 'flex', gap: '16px' }}>
                            <button onClick={() => setShowMaterialModal(false)} style={{ flex: 1, padding: '16px', borderRadius: '12px', border: 'none', background: 'var(--border-color)', color: '#374151', fontWeight: '700', cursor: 'pointer' }}>Cancel</button>
                        ...
                        {scanningRowIndex !== null && (
                            <div style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', alignItems: 'center' }}>
                                    ...
                                    <button onClick={stopScanner} style={{ background: '#f1f5f9', border: 'none', width: '36px', height: '36px', borderRadius: '50%', fontSize: '20px', cursor: 'pointer', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                                </div>
